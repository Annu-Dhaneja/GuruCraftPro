...
if not DB_URL or "sqlite" in DB_URL:
    DB_URL = input("Please enter your database connection string: ").strip()
    if DB_URL:
...
if not DB_URL:
    print("[ERROR] No DATABASE_URL specified. Exiting.")
    sys.exit(1)
...
def sync_production_data():
    ...
    try:
        ...
        print("[SYNC] Verifying tables exist...")
        Base.metadata.create_all(bind=engine)

        # --- 1. SYNC GURU JI ART DATA ---
        print("[SYNC] Updating Guru Ji Art content...")
        guruji_data = {
            "slug": "guru-ji-art",
            "hero": {
        ...
        print("[SYNC] Upserting guru-ji-art content using relational CMS (V3 SSOT)...")
        cms_repository.update_page_content(db, "guru-ji-art", guruji_data)
        
        # --- 2. SYNC VANTAGE ECOM DATA ---
        print("[SYNC] Updating VantageEcom content...")
        vantage_data = {
            "slug": "vantage-ecom",
            "hero": {
                "title": "VantageEcom",
        ...
        print("[SYNC] Upserting vantage-ecom content using relational CMS (V3 SSOT)...")
