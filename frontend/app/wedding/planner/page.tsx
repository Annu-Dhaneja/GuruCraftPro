...
export default function WeddingPlannerPage() {
    ...
    const phases = [
        { id: "planning", label: "Tasks", icon: Calendar, description: "Manage milestones.", items: tasks.map((t) => ({ name: t.title, status: t.status, desc: t.category })), stats: `${tasks.filter((t) => t.status === "completed").length}/${tasks.length}`, href: "/wedding/planner/tasks" },
        { id: "guests", label: "Guests", icon: Users, description: "Guest list & RSVP.", items: guests.slice(0, 5).map((g) => ({ name: g.name, status: g.status, desc: g.category })), stats: `${guests.length} Total`, href: "/wedding/planner/guests" },
        { id: "vendors", label: "Vendors", icon: Heart, description: "Contracts & payments.", items: vendors.map((v) => ({ name: v.name, status: v.status, desc: v.category })), stats: `${vendors.length} Hired`, href: "/wedding/planner/vendors" },
        { id: "budget", label: "Budget", icon: Wallet, description: "Financial tracking.", items: budget.map((b) => ({ name: b.category, status: "active", desc: `₹${(b.spent_amount / 1000).toFixed(0)}k` })), stats: stats?.budget_spent || "₹0", href: "/wedding/planner/budget" },
    ];
