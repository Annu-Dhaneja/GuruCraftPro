...
export function GurujiArtEcommerce({ data }: { data?: GurujiArtData }) {
  ...
  const defaultProducts: Product[] = [
    {
      ...
      description: "Pre-loaded with 100+ Bhajans, Mantras and Aarti. Premium wood finish with gold leaf accents.",
      price: "₹4,999",
    ...
    },
    {
        ...
        description: "High-vibrational digital painting optimized for 8K displays. Features a radiant amber blessing field.",
        price: "₹1,499",
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white selection:bg-amber-500/30 overflow-x-hidden">
      {/* ── Fixed Header ───────────────────────────── */}
      <header className="fixed top-0 left-0 right-0 z-[100] bg-zinc-950/80 backdrop-blur-xl border-b border-white/5">
        <div className="container mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4">
             <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-400 to-orange-600 flex items-center justify-center shadow-lg shadow-amber-600/20">
                <Sparkles className="w-6 h-6 text-white" />
             </div>
             <span className="text-xl font-black uppercase tracking-tighter italic">Divine <span className="text-amber-500">Studio</span></span>
          </div>
          ...
          <div className="hidden md:flex items-center gap-8">
            <div className="relative group">
               ...
               <input 
                 ...
                 className="bg-white/5 border border-white/10 rounded-full py-2.5 pl-12 pr-6 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/50 w-64 transition-all"
               />
            </div>
            ...
            <Button variant="ghost" className="relative h-12 w-12 rounded-full hover:bg-white/5 transition-all">
              ...
              {cartCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-amber-600 text-white text-[10px] h-5 w-5 flex items-center justify-center rounded-full font-black border-2 border-zinc-950">
                  {cartCount}
                </span>
              )}
            </Button>
        ...
        <div className="container mx-auto">
          <div className="max-w-4xl">
            ...
            >
              ...
              <span className="text-amber-500 text-[10px] font-black uppercase tracking-[0.4em]">Official Collection</span>
            </motion.div>
          ...
          <aside className="w-full md:w-64 space-y-12">
            <div>
              <h3 className="text-xs font-black uppercase tracking-[0.3em] mb-8 flex items-center gap-2">
                <Filter className="w-4 h-4 text-amber-500" /> Categories
              </h3>
                ...
                {categories.map((cat) => (
                  <button
                    ...
                    className={`block text-lg font-bold transition-all hover:translate-x-2 ${
                      activeCategory === cat ? "text-amber-500 italic translate-x-4" : "text-zinc-500 hover:text-white"
                    }`}
                  >
                    {cat}
                  </button>
            ...
            <div className="p-8 rounded-[2rem] bg-amber-500/5 border border-amber-500/10 backdrop-blur-3xl">
               ...
               <Button variant="outline" className="w-full border-amber-500/30 text-amber-500 hover:bg-amber-500 hover:text-white rounded-xl h-12 text-xs font-black uppercase tracking-widest transition-all">
                 Learn More
               </Button>
            </div>
          </aside>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
              {filteredProducts.map((product, i) => (
                <motion.div
                  ...
                  className="group relative flex flex-col bg-zinc-900/50 border border-white/5 rounded-[2.5rem] overflow-hidden hover:border-amber-500/30 transition-all duration-500 shadow-2xl"
                >
                  {/* Image Container */}
                  <div className="relative aspect-[4/5] overflow-hidden">
                    ...
                    <button className="absolute top-6 right-6 h-10 w-10 rounded-full bg-white/5 backdrop-blur-md border border-white/10 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all hover:bg-amber-600 hover:border-amber-600 group/btn">
                  ...
                  <div className="p-10 flex flex-col flex-grow">
                    <div className="flex justify-between items-start mb-4">
                      <h3 className="text-2xl font-black uppercase italic tracking-tight text-white group-hover:text-amber-500 transition-colors leading-none">
                        {product.title}
                      </h3>
                    </div>
                    ...
                    <div className="mt-auto">
                       <div className="flex items-center justify-between mb-8">
                          ...
                          <div className="flex items-center gap-1">
                             ...
                             <span className="text-xs font-black text-amber-500">{product.rating || "5.0"}</span>
                          </div>
                       </div>
                       ...
                       <div className="grid grid-cols-2 gap-4">
                          ...
                          <Button className="h-14 rounded-2xl bg-amber-600 text-white hover:bg-white hover:text-black font-black uppercase text-[10px] tracking-widest transition-all shadow-lg shadow-amber-600/20">
                             Buy Now
                          </Button>
                       </div>
                    </div>
      ...
      <section className="py-64 text-center relative overflow-hidden">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-amber-600/5 blur-[200px] rounded-full pointer-events-none" />
        <motion.div
        ...
        >
          <Sparkles className="w-16 h-16 text-amber-500/50 mx-auto mb-12 animate-pulse" />
          <h2 className="text-5xl md:text-7xl font-black mb-12 tracking-tighter uppercase italic leading-none">
            Ready for <br /> <span className="text-shimmer bg-gradient-to-r from-amber-100 via-orange-300 to-amber-500 bg-clip-text text-transparent">Ascension?</span>
          </h2>
          ...
          <Button size="lg" className="h-24 px-16 rounded-2xl bg-white text-black hover:bg-amber-600 hover:text-white text-2xl font-black uppercase italic transition-all group" asChild>
