...
export function ContactProcess({ data }: { data?: any }) {
    const processSteps = data?.steps && data.steps.length > 0 ? data.steps : defaultSteps;

    return (
        <section className="py-20 bg-muted/20 border-y border-border">
            <div className="container mx-auto px-4 md:px-6">
                <div className="text-center mb-12">
                    <h2 className="text-2xl font-bold mb-2">{data?.title || "What happens next?"}</h2>
                    <p className="text-muted-foreground">{data?.description || "A transparent look at our onboarding process."}</p>
                </div>
                ...
                <div className="flex flex-col md:flex-row justify-center items-center gap-8 md:gap-4 relative max-w-4xl mx-auto">
                    ...
                    {processSteps.map((step: any, i: number) => {
                        const IconComp = typeof step.icon === 'string' ? MessageSquare : step.icon; // fallback
                        return (
