...
export function ServicesPreview({ data }: { data?: any }) {
    const title = data?.title || "What We Create";
    const previewServices = data?.services || services;
    return (
            ...
            <div className="max-w-4xl mx-auto text-center">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
                    {previewServices.map((item: any) => {
                        const Icon = services.find(s => s.title === item.title)?.icon || PenTool;
                        return (
